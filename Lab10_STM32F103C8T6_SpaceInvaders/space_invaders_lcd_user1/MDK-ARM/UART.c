...
volatile uint8_t uart_received;
...
void UART2_SendChar(char c) {
    uint8_t next_write_2 = (tx_write_2 + 1) % BUFFER_SIZE;
    if (next_write_2 != tx_read_2) {  // Ki?m tra không tràn buffer truy?n
        tx_buffer_2[tx_write_2] = (uint8_t)c;
        tx_write_2 = next_write_2;
        USART2->CR1 |= USART_CR1_TXEIE;  // Kích ho?t ng?t TXE
    }
}
...
void USART2_IRQHandler (void){
    if (((USART2->SR & USART_SR_TXE) && (USART2->CR1 & USART_CR1_TXEIE))){
        if (tx_read_2 != tx_write_2){
            ...
            USART2->CR1 &= ~USART_CR1_TXEIE; // T?t ng?t TXE khi không còn d? li?u d? g?i
    }
    ...
    if ((USART2->SR & USART_SR_RXNE)){
        ...
        if(next_write_2 != rx_read_2) { // Check buffer overflow
            ...
            rx_write_2 = next_write_2;
						uart_received_2 = 1; // Set the flag after writing to buffer
        }
    }
}